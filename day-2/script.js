...
function partThreeB () {
  ...
  if(color === "red") {
    ...
    messageParagraph.innerHTML = `Ooo, feisty aren't we?`;
  // 3. check for the next color
  } else if(color === "brown") {
    // 4. give the message for the next color
    messageParagraph.innerHTML = `Go to the bathroom, dude.`; 
  } else {
  // 5. otherwise, ask for another color
  messageParagraph.innerHTML = `Sorry, I only work for Red or Brown. Try one of those.`;
  }
}
...
function partFourB () {
  ...
  if(playerOne === "rock" && playerTwo === "rock"){
    ...
    messageParagraph.innerHTML = `What are you doing...?`
  }
}
